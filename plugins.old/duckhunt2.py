...
from sqlalchemy import Table, Column, String, Integer, DateTime, PrimaryKeyConstraint, desc
...
@hook.command("save", autohelp=False)
def save(nick, chan, message, db, conn, notice):
    """when there is a killed duck use this command to save it."""
    global game_status, scripters
    if chan in opt_out:
        return
    network = conn.name
    score = ""
    out = ""
    miss = ["BUMMER! You failed to save the duck!", "Your gun jammed!", "Better luck next time.", "WTF!? Who are you Dick Cheney?" ]
    if not game_status[network][chan]['game_on']:
        return "There is no activehunt right now. Use .starthunt to start a game."
    elif game_status[network][chan]['duck_status'] != 1:
        if game_status[network][chan]['no_duck_kick'] == 1:
            out = "KICK {} {} There is no duck! What are you trying to save?".format(chan, nick)
            conn.send(out)
            return
        return "There is no duck. What are you shooting at?"
    else: 
...
def befriend(nick, chan, message, db, conn, notice):
    ...
    if not game_status[network][chan]['game_on']:
        ...
        game_status[network][chan]['befriend_time'] = time()
        deploy = game_status[network][chan]['duck_time']
        befriend = game_status[network][chan]['befriend_time']
        if nick.lower() in scripters:
            if scripters[nick.lower()] > shoot:
                notice("You are in a cool down period, you can try again in {} seconds.".format(str(scripters[nick.lower()] - shoot)))
                return
...
@hook.command("duckmerge", permissions=["duckmerge"])
...
@hook.command("killsmerge", permissions=["duckmerge"])
...
@hook.command("friendsmerge", permissions=["duckmerge"])
